...
def filter_series(series,start,end,freq):
    """
    Returns a list of elements of a pandas series if the index lies between the start and end points.
    
    Args:
        series (pandas Series): Index must be datetimeindex.
        
        start (datetime): Start of interval (closed).
        
        end (datetime): End of interval (closed).
        
        freq (int): Frequency of series datetimeindex in minutes.
        
    Returns:
        mylist (list): List of elements of series with index between the start and end times.
    ...
    if len(mylist) < expected_intervals:
        print(f"WARNING: You don't have full coverage of your interval. You will need to load more data. Your coverage is:\n{100*len(mylist)/expected_intervals:.2f}% of {series.index.min()},{series.index.max()}")
    else:
        print(f"Number of intervals to read: {len(mylist)}\n{end - start}")
...
end = dt.datetime(2021,3,6)
days_targ = 5 # number of days we want
...
my_files = filter_series(files_fcas,start,end,30)

# Take a sample of days
files_fcas.index.name = 'Timestamp'
files_fcas = files_fcas[files_fcas.isin(my_files)]
files_fcas.name = 'File'
files_fcas = files_fcas.to_frame().reset_index()

# Create a column with the days we have
files_fcas['Days'] = files_fcas['Timestamp'].apply(lambda x: dt.datetime(x.year, x.month, x.day))


days = set(files_fcas['Days']) # set of days we have

days_remove = int(len(days) - days_targ) # number of days to remove

# Remove days_remove number of days randomly
for d in range(0,days_remove):
    removed = random.choice(tuple(days))
    days.remove(removed)
    
files_fcas = files_fcas[files_fcas['Days'].isin(list(days))]
...
fcas_5min_all = []
for day in days:
    toRead = list(files_fcas[files_fcas['Days'] == day]['File'])
    ...
    # #%% Use duid mapping to identify the bess duid and combine gens with loads
    print("Use duid mapping to identify the bess duid and combine gens with loads")
    # Combine gens and loads into a single duid where negative is load, positive is gen 
    ...
    # #%% Retrieve the correct dispatchload files based on the fcas data we're processing
    print("Retrieve the correct dispatchload files based on the fcas data we're processing")
    date_format = "%Y%m%d"
    ...
    toRead = list(files_disload[files_disload.index == day])
    data_disload = unzipcsvs(path_disload,toRead,header=1) # read in the raw data
    ...
    fcas_5min_all.append(fcas_5min) # collect fcas_5min

fcas_5min_all = pd.concat(fcas_5min_all) # concatenate
#%% Plot the 5min data
multiindex = ['Timestamp','duid pre']
toPlot = fcas_5min_all.copy().set_index(multiindex)[['RAISEREG','LOWERREG','RAISE_frac','LOWER_frac']].stack().reset_index().rename({f'level_{len(multiindex)}':'Variable',0:'Value'},axis=1)
fig = px.scatter(toPlot,x='Timestamp',y='Value',color='duid pre',facet_row='Variable').update_yaxes(matches=None)
plot(fig)

#%% Plot a histogram of RAISE_frac and LOWER_frac
toPlot_hist = toPlot.copy()[toPlot['Variable'].isin(['RAISE_frac','LOWER_frac'])]
toPlot_hist = toPlot_hist.groupby(['Timestamp','duid pre']).sum().reset_index()

fig = px.histogram(toPlot_hist,x='Value',color='duid pre').update_yaxes(matches=None).update_layout(xaxis={'range':[-1,1]})
plot(fig)
